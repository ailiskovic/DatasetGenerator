...
import com.example.datasetgeneratiorapp.Utils.UnivariateHawkesProcess;
...
public class CreateNewDatasetController {
    ...
    public List<String[]>  generateInputData(ArrayList<Element> elements, int numberOfRows, int numberOfColumns)
    {
        ...
        Map<Integer,Double[]> hawkessMap=new HashMap<Integer,Double[]>();
        ...
        // generate Hawkess process elements
        for(int i = 0; i < numberOfColumns; i++)
        {
            Element currentElement = elements.get(i);
            if(currentElement instanceof HawkessProcessElement element)
            {
                Map<Double, Double>[] simulate = UnivariateHawkesProcess.simulate(element.getLambdaValue(),
                        element.getAlphaValue(), element.getBetaValue(), numberOfRows);
                for(Map<Double, Double> mapElements : simulate)
                {
                    System.out.println(mapElements);
                    for(int j = 0; j < mapElements.size(); j++)
                    {
                        Map<Double, Double> doubleElements = mapElements;
                        int counter = 0;
                        for(Double doubleElement : doubleElements.values()) {
                            if (counter >= numberOfRows)
                                break;
                            element.addPreviousValue(doubleElement);
...
//                        element.addPreviousValue(value);
//                    }
                    double value = element.getPreviousValues().get(i);
                    rowList.add(String.valueOf(value));
                }
