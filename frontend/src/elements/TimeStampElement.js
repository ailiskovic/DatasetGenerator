...
/**
 * Function which creates a new time stamp element. 
 * @constructor
 */
export default function TimeStampElement({id, type, name, dateValue, unitValue, timeValue, increase,  handleDelete, handleUpArrow, handleDownArrow, handleOnChange,
  ...
  /** Function which toggle time change value */
  const toggle = (id) => {
    setModal(!modal)
  ...
  /** Fuction which update time value */
  ...
  return (
    ...
    <tr >
        ...
        <td>
        <DatePicker  size='sm' selected={dateValue} onChange={(date) => handleOnDateChange(date)} />
        </td>
        <td>
        <InputGroup>
        <Input
